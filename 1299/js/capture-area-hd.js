...
`;
    ...
    editButton.addEventListener("mouseout", () => editButton.style.cssText = buttonStyles + "background-color: #f5f0f0; color: black;");
                ...
                for (let c = 0; c < 3; c++) { // RGB channels
                    ...
                    for (let ky = -1; ky <= 1; ky++) {
                        for (let kx = -1; kx <= 1; kx++) {
                            ...
                            sum += data[pixelIdx] * sharpenKernel[(ky + 1) * 3 + (kx + 1)];
                        }
                    }
                    tempData[idx + c] = Math.min(255, Math.max(0, sum));
                }
                tempData[idx + 3] = data[idx + 3]; // Alpha
            }
        }
            ...
            chrome.runtime.sendMessage(
                ...
                (response) => {
                    if (response.success) {
                        ...
                        ctx.imageSmoothingEnabled = false; // Disable smoothing for crisp HD scaling
                        // Factor in devicePixelRatio for high-resolution screens
                        const dpr = window.devicePixelRatio || 1;
                        const hdScale = 4; // Increased for full HD+ upscaling
                        ...
                        img.onload = () => {
                            ...
                            applySharpen(ctx, canvas.width, canvas.height); // Apply sharpening filter
