...
(function () {
    ...
    function multiPassCapture(selLeft, selTop, selWidth, selHeight, cb) {
        // We will chunk the selection in tiles based on window.innerWidth, window.innerHeight
        // You may want to subtract some margin if your extension’s capturing region
        // includes scrollbars, header bars, etc.
        const viewportWidth  = window.innerWidth;
        const viewportHeight = window.innerHeight;

        // Create a final canvas that can hold the entire selection
        const finalCanvas = document.createElement("canvas");
        finalCanvas.width  = selWidth;
        finalCanvas.height = selHeight;
        const finalCtx = finalCanvas.getContext("2d");

        // Build a list of “tiles” we need to capture
        const tiles = [];
    //    const scrollStepX = Math.min(viewportWidth,  800);  // arbitrary step sizes
      //  const scrollStepY = Math.min(viewportHeight, 600);

       const scrollStepX = viewportWidth;  // arbitrary step sizes
        const scrollStepY = viewportHeight;
        ...
        for (let y = 0; y < selHeight; y += scrollStepY) {
            for (let x = 0; x < selWidth; x += scrollStepX) {
                tiles.push({
                    scrollX: selLeft + x,
                    scrollY: selTop + y,
                    // how wide/tall to capture
                    width:  Math.min(scrollStepX, selWidth - x),
                    height: Math.min(scrollStepY, selHeight - y),
                    finalX: x,   // where in final canvas we place it
                    finalY: y
                });
            }
        }

        // A function to process each tile in sequence
        let index = 0;
        function processNext() {
            ...
            // Wait a bit for scroll to finish (some pages take time)
            setTimeout(() => {
                ...
                chrome.runtime.sendMessage(
                    ...
                    (response) => {
                        if (response && response.success && response.dataUrl) {
                            // We have a full viewport screenshot in response.dataUrl
                            // Draw only the portion that corresponds to the tile
                            const tempImg = new Image();
                            tempImg.onload = () => {
                                // Determine the offset of the tile within the *current* viewport screenshot
                                // We scrolled to (tile.scrollX, tile.scrollY) in doc coords
                                // but the screenshot might correspond exactly to the viewport
                                // at that offset.
                                // Usually, you can simply draw the entire image if your background
                                // script already cropped to the viewport, but often you need
                                // to figure out the difference between (tile.scrollX, window.scrollX) etc.
                                const dx = tile.scrollX - window.scrollX; // how far into the screenshot we start
                                const dy = tile.scrollY - window.scrollY;

                                // Actually draw that portion into the final canvas
                                finalCtx.drawImage(
                                    tempImg,
                                    dx,
                                    dy,
            ...
            }, 2000); // 400ms scroll “settle” time - adjust as needed
        }
	...
	// auto eedit clicks starts here  hideoverlay 
