chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "getPageUrl") {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      sendResponse({ pageUrl: activeTab.url });
    });
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "show_screen_area") {
     sendResponse({ success: true }); // Required to prevent port closing
    return true; // Keeps the message port open for sendResponse
  }

  if (message.type === "hidden_scroll") {
...
chrome.runtime.onInstalled.addListener(() => {
    chrome.storage.local.get("collections", (data) => {
        const needsMigration = !data.collections || 
            data.collections.some(c => !c.id || !Array.isArray(c.bookmarks));

        if (needsMigration) {
            const migrated = (data.collections || []).map((c, index) => ({
                id: c.id || `col-${Date.now()}-${index}`,
                name: c.name || 'Unnamed Collection',
                bookmarks: Array.isArray(c.bookmarks) ? c.bookmarks.map(b => ({
                    id: b.id || `bm-${Date.now()}-${Math.random()}`,
                    url: b.url || '',
                    title: b.title || 'Untitled Bookmark'
                })) : []
            }));
            
            chrome.storage.local.set({ collections: migrated }, () => {
...
// trying context meu starts here
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  ...
  //    break;
     ...
     break;
      ...
      chrome.tabs.create({ url: chrome.runtime.getURL("/recording/pages/popup/popup14.html") });
        return; // Exit the handler for this case
    default:
      handleSelection(info.menuItemId, info.selectionText, tab.id);
  }
  chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    if (mode) {
      chrome.tabs.sendMessage(tabs[0].id, { cmd: "getImgSrc", mode: mode });
    }
  });
});
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  console.log(request); //获取参数
  if (request.cmd == "getParams") {
    sendResponse({ params: { imgSrc: imgSrc, mode: mode } });
  } //设置参数
  if (request.cmd == "mode") {
    mode = request.mode; //向指定tab发送消息
chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
  if (tabs[0] && tabs[0].id) {
    chrome.tabs.sendMessage(tabs[0].id, { cmd: "getImgSrc", mode: mode });
  } else {
...
// const htmlFileyy = mode === "selecto" ? "screenshot_old.html" : "screenshot.html";

if (request.cmd === "imgSrc") {
    imgSrc = request.imgSrc;

    getMode(function (mode) {
         const htmlFileyy = mode === "selecto" ? "screenshot_old.html" : "screenshot.html";

        if (mode !== "screenrecorder") {
            chrome.tabs.create(
                {
                    url: chrome.runtime.getURL(`/html/${htmlFileyy}?id=10&mode=${mode}`),
                },
                function () {
                    sendResponse({});
                }
            ...
            console.log("Mode is screenrecorder. Tab creation skipped.");
            sendResponse({});
        }
    });
...
else if (request.cmd === "open_dashboard") {
    chrome.tabs.create({ url: chrome.runtime.getURL("/html/dashboard.html") });
}
else if (request.cmd === "scrsht_history") {
    chrome.tabs.create({ url: chrome.runtime.getURL("/html/scrsht_history.html") });
}
else if (request.cmd === "open_todo_dashboard") {
    chrome.tabs.create({ url: chrome.runtime.getURL("/html/todo.html") });
}
...
function handleSelection(task, selection, tabId) {
  if (!selection) {
    return;
  }
  ...
  switch (task) {
    ...
    case "selection":
      chrome.scripting.executeScript({
        target: {tabId: tabId},
        func: () => { return prompt("What's the context?") },
      }).then(results => {
        let context = results[0]?.result
        if (context) {
          searchUrl = `https://www.chatgpt.com/?q=${context}%0A%0A${query}`;
    ...
    case "translateSelection":
      chrome.scripting.executeScript({
        target: {tabId: tabId},
        func: () => { return prompt("Translate to which language?", "English") },
      }, results => {
        let language = results[0]?.result;
        if (language) {
          searchUrl = `https://www.chatgpt.com/?q=${encodeURIComponent("Translate the following to ") + language + ":"}%0A%0A${query}`;
          chrome.windows.create({
            url: searchUrl,
            type: "popup",
          });
    ...
    // CASES WITHOUT PROMPTS
    case "summarizeSelection":
      searchUrl = `https://www.chatgpt.com/?q=${encodeURIComponent("Summarize the following:")}%0A%0A${query}`;
      break;
    case "explainSelection":
      searchUrl = `https://www.chatgpt.com/?q=${encodeURIComponent("Explain the following:")}%0A%0A${query}`;
      break;
    case "answerSelection":
      searchUrl = `https://www.chatgpt.com/?q=${encodeURIComponent("Answer the following:")}%0A%0A${query}`;
      break;
    case "expertsExplain":
      searchUrl = `https://www.chatgpt.com/g/g-6AIsip2Fo-learn-anything`;
      manualSend = true;
      break;
  }
...
function sendPromptManually(query) {
  waitForElement("#prompt-textarea").then(element => {
    element.innerHTML = query;
    waitForElement("button[data-testid=send-button]").then(element => {
      element.click();
    });
  });
}
...
D.prototype.create = function () {
  var progrr = Math.floor(Math.random() * 65) + 10;
  var a = this,
    b = {
      type: "progress",
      title: "scrsht",
...
var K = (function () {
  function a() {
    return "https://scrsht.com/touch/chrome_extn/screenshot/screesnhot_v1.php";
  }
  return {
    save: function (b, c) {
      b &&
        "undefined" != typeof b &&
        "undefined" != typeof b.img_url &&
        $.ajax({
          type: "POST",
          url: a(),
          data: JSON.stringify({
...
if ("function" !== typeof L)
  var L = (function () {
    function a(a, k) {
      ...
      e = l(e, d, f, h, k[13], 21, 1309151649);
    ...
    function q(b) {
      ...
      d[e >> 2] |= 128 << (e % 4 << 3);
      if (55 < e) for (a(b, d), e = 0; 16 > e; e++) d[e] = 0;
      d[14] = 8 * c;
      a(b, d);
...
M = (function (a) {
  ...
  (function () {
    ...
    var b = null,
      q = a;
    "undefined" !== typeof window && (b = window.crypto || window.Fa)
      ? b.getRandomValues &&
...
N = {
  ...
  H: function (a) {
    a &&
      chrome.storage.local.get("app_token", function (b) {
        a("app_token" in b ? b.app_token : null);
      });
  },
  ya: function (a) {
    a &&
      chrome.storage.local.remove("app_token", function () {
        a();
      });
  },
  D: function (a) {
    a &&
      chrome.storage.local.get("app_id", function (b) {
...
function P(a, b) {
  ...
  this.M = "5CE3DF4D45AC";
  //this.ta = "http://upload.scrsht.com/upload/{time}/{hash}/"
  this.ta = "https://scrsht.com/touch/chrome_extn/screenshot/upload_s.php?/{time}/{hash}/";
}
A.extend(P, O);
P.prototype.upload = function () {
  ...
  b = this.ta.replace("{time}", b).replace("{hash}", g);
  ...
  this.L = fetch(b, options)
    .then((res) => res.text())
    .then((res) => {
      var re_title = new RegExp("<share>[\n\rs]*(.*)[\n\rs]*</share>", "gmi");
      var sharelinkks = re_title.exec(res)[1];

      //function Greeter() {

      sendMessage: function gty() {
        const objfrg = a.b;
        timeryyy = setTimeout(function () {
          chrome.notifications.clear(objfrg);
        }, 100);
      }
      gty();
      //}
...
var E = (function () {
    ...
    g(a, function () {
      if (C[a] && C[a].cmdAfterUpload) {
        ...
        switch (C[a].cmdAfterUpload) {
          case "search_google":
            chrome.tabs.create({
              url: "http://www.google.com/searchbyimage?image_url=" + b + "/direct",
            });
            break;
          case "share_twitter":
            chrome.tabs.create({
              url: "http://twitter.com/home?source=scrsht&status=" + b + "%20",
            });
            break;
          case "share_whatsapp":
            chrome.tabs.create({
              url: "https://web.whatsapp.com/send?text=" + b + "%20",
            });
            break;
          case "share_facebook":
            chrome.tabs.create({
  ...
  function r() {
    n(function (a) {
      ...
      chrome.tabs.create(
        {
          url: chrome.runtime.getURL("/html/screenshot.html?id=" + b),
        },
        function () {}
      );
    });
  ...
  chrome.contextMenus.onClicked.addListener((clickData) => {
    if (clickData.menuItemId == "scrshtnow") {
      r();
    }
  });
    ...
    N.D(function (c) {
      chrome.runtime.getPlatformInfo(function (d) {
        ...
        K.ha(a, c, d, function (c) {
          !0 === c.success && "app_token" in c && "" !== c.app_token
            ? N.Da(a, function () {
  ...
  return {
    sa: function () {
      ...
      //	chrome.tabs.getCurrent("copy", a);

      var c = chrome.runtime.getManifest().version;
      "undefined" === typeof window.localStorage.scrsht_version ? (window.localStorage.ever_updated = "no") : window.localStorage.scrsht_version != c && (window.localStorage.ever_updated = "yes");
      window.localStorage.scrsht_version = c;
    },
        ...
        switch (a.name) {
          case Y:
            ...
            timer = setTimeout(function () {
              chrome.notifications.clear(a.id);
            }, 100000);
